...
Heading（Issue #27）に`color`propを追加するにあたり、任意のCSS値（例: `color="#ff0000"`）を許容するか、`packages/tokens`で定義されたキーのみに制約するかを決める必要があった。この決定はcolorに限らず、今後spacing・radius等トークン化された値に対応するpropsを持つ全コンポーネントに適用される原則になる。
...
- 当初Headingに`marginBottom`prop（spacingトークン制約）を追加したが、ADR 0012でBox/Stackによるレイアウト合成を導入したことに伴い削除した。余白調整はコンポーネント自身のpropsではなく、Box/Stack側で行う方針に統一したため（`packages/tokens/src/spacing.ts`自体はBox/Stackのstyle propsが引き続き使用するため残す）
