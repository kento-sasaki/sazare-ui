...
- 個別コンポーネントは原則として自身の余白（margin）用propsを持たない。Headingには当初`marginBottom`prop（spacingトークン制約）を用意したが、本ADRの決定に伴い削除した（ADR 0011）。余白調整は常にBox/Stackで行い、コンポーネントごとに重複した余白propsを持たせない
