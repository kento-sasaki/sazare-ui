...
1. **標準セット**: 概要 / Anatomy / Props API / 状態 / インタラクション / バリアント / アクセシビリティ / キーボード操作 / 不変条件 / 例外ケース / 使用例
2. **デザイントークンのマッピング**: このコンポーネントで使うトークンを明示
3. **テストシナリオ**: Given/When/Then 形式

「関連コンポーネント・依存」は含めない方針。

**インタラクション章の位置づけ**: `sdd-workflow.md` §3.4 の 1:1 翻訳ルールが「状態表」と「インタラクション表」を別概念として扱うため、独立章とする。状態が変わる操作は両章に重複登場しうる（§4 は「結果として何状態か」、§5 は「副作用として何が起きるか」）。
...
| 2026-05-21 | `components/_template.md` 作成（Issue #8）に伴い、Spec 標準セットに「インタラクション」章を追加。sdd-workflow.md §3.4 の「インタラクション表」要件を独立章として明文化 |
| 2026-05-27 | `vision.md` 作成（Issue #9）。一次目的=個人再利用/npm 副次、想定ユーザー=自分+Claude、成功基準 4 軸（ミニマル/WCAG/トークン・API/技術基準）、非ゴール 3 点（データ可視化/Figma カンプ/汎用 OSS 網羅）を確定。designPrinciple §1・implementationPlan §5 を吸収 |
| 2026-05-27 | `principles/design-principles.md` 作成（Issue #10）。判定方式=ハイブリッド（問い＋緩い目安）、装飾=条件付き許可（影=エレベーション限定/グラデーション=原則禁止/枠線=区切り不十分時）、アクセント=役割ベース（1 画面 1 主役、セマンティックは別軸）、タイポ／アイコン=原則のみ（値はトークンに委譲）を確定。designPrinciple §2 を吸収、A11y 部分は accessibility.md へ委譲 |
